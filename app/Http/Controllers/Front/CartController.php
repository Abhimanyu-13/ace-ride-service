                    ...
                    if (in_array($action,['delivery','appointment','on_demand'])) {
                        ...
                        if (!empty($prod->product->Requires_last_mile) && ($prod->product->Requires_last_mile == 1)) {
                            ...
                            if (isset($deliveryCharges) && !empty($deliveryCharges)) {
                                ...
                                CartDeliveryFee::updateOrCreate(['cart_id' => $cart->id, 'vendor_id' => $vendorData->vendor->id], ['delivery_fee' => $deliveryCharges, 'primary_currency_delivery_fee' => $primaryDeliveryCharges, 'delivery_duration' => $deliveryDuration,'shipping_delivery_type' => $dtype[0]??'D','courier_id'=>$dtype[1]??'0']);
                            }
