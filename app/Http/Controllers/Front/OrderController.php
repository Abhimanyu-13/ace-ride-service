...
class OrderController extends FrontController
{
    ...
    public function sendOrderNotification($id, $vendorIds)
    {
        ...
        // if ($notification_content && ! empty($token) && ! empty($client_preferences->fcm_server_key)) {
            if ($notification_content && ! empty($client_preferences->fcm_server_key)) {
                \Log::info("entered notification 2 =>>>>>>");
        ...
        foreach ($devices as $device) {
            // \Log::info("super admin =>>>>>>"  . $device);
            // $data = [
            //     "message" => [
            //         "token" => "e1bjut0fYz_RhriLmIL7Hf:APA91bH6Ydwz2WDwbaK4TRe6u13OU4Bzk9G0MASbQnzQvlDN1AhuBGy5ToIm17UpEkpc-yMRv9dCYjnnOKdfp0GDVNmu8c7vMon8bc1-HUuzsH5HLerh_2k",
            //         "notification" => [
        ...
        // $from = env('FIREBASE_SERVER_KEY');
            ...
            FacadesLog::info("notification data =>>>>>> 1");
            sendFcmCurlRequest($data);
        }
    }
    ...
    {
        \Log::info("entered vendor notification");
        ...
        if ($notification_content) {
            ...
            foreach ($devices as $device_token) {
                // $data['message']['token'] = $device_token; // Set the token for the current device
                $data['tokens'][] = $device_token; // Set the token for the current device
                // if (!empty($from)) {
                    // Send the notification for the current device
                    // }
            }
            sendFcmCurlRequest($data);
            ...
            if (! empty($vendorAppUserDevices)) {
                ...
                // Loop through each vendor app device token and send notification
                foreach ($vendorAppUserDevices as $vendor_device_token) {
                    // $data['message']['token'] = $vendor_device_token; // Set the token for the current vendor app device
                    $data['tokens'][] = $vendor_device_token; // Set the token for the current vendor app device
        ...
        try {
            ...
            if ($dispatch_domain && $dispatch_domain != false) {
                ...
                foreach ($driver_registration_documents as $driver_registration_document) {
                    if (isset($driver_registration_document->is_required) && $driver_registration_document->is_required == 1) {
