    ...
    class OrderController extends BaseController
    {
                            ...
                            foreach ($vendor_cart_products as $vendor_cart_product) {
                                ...
                                if ($action == 'delivery' || $action == 'on_demand') {
                                    ...
                                    if ((!empty($vendor_cart_product->product->Requires_last_mile)) && ($vendor_cart_product->product->Requires_last_mile == 1) || isset($deliver_fee_data)) {
                                        ...
                                        if ($deliver_fee_data):
                                            $delivery_fee  = $deliver_fee_data->primary_currency_delivery_fee ?? 0.00;
                                        ...
                                        if (!empty($delivery_fee) && $delivery_count == 0) {
                                            ...
                                            } elseif ($vendor_cart_product->vendor->timeofLineOfSightDistance > 0) {
            ...
            if ($luxury_option_id == 6) { // only for on_demand type
                ...
                if ($dispatch_domain_OnDemand && $dispatch_domain_OnDemand != false) {
                    ...
                    foreach ($checkdeliveryFeeAdded->products as $key => $prod) {
                        ...
                        } elseif (isset($prod->product_dispatcher_tag) && !empty($prod->product_dispatcher_tag) && $prod->product->category->categoryDetail->type_id == 8) {
        ...
        public function sendOrderNotification($id)
        {
            ...
            $client_preferences = ClientPreference::select('fcm_server_key', 'fcm_project_id', 'favicon')->first();
            //$from = env('FIREBASE_SERVER_KEY');
            $notification_content = NotificationTemplate::where('id', 1)->first();
            if ($notification_content && !empty($token) && !empty($client_preferences->fcm_project_id)) {
        ...
        public function orderlistLoop($orders, $user, $request)
        {
            ...
            foreach ($orders as $order) {
                ...
                $order->payable_amount = decimal_format($order->orderDetail->payable_amount) * $clientCurrency->doller_compare;
            ...
            try {
                ...
                if ($order) {
                    ...
                    foreach ($order->vendors as $vendor) {
                        ...
                        $vendor->payable_amount = $vendor->payable_amount * $clientCurrency->doller_compare;
                        $vendor->delivery_fee = $vendor->delivery_fee * $clientCurrency->doller_compare;
                        foreach ($vendor->products as $product) {
                            ...
                            if (!empty($product->addon)) {
                                foreach ($product->addon as $k => $addon) {
                                    ...
                                    $product_addons[$k]['price'] = $opt_price_in_doller_compare;
                            ...
                            $product->price = $product->price * $clientCurrency->doller_compare;
                ...
                // dd($clientCurrency);
                $order['total_amount'] = $order['total_amount'] * $clientCurrency->doller_compare;
                $order['payable_amount'] = $order['payable_amount'] * $clientCurrency->doller_compare;
                $order['total_delivery_fee'] = $order['total_delivery_fee'] * $clientCurrency->doller_compare;
                $order['subtotal_amount'] = $order['subtotal_amount'] * $clientCurrency->doller_compare;
            ...
            try {
                ...
                if ($orderData->total_other_taxes != '') {
                    foreach (explode(",", $orderData->total_other_taxes) as $row) {
                        ...
                        } elseif ($check_row == "tax_fixed_fee") {
                            $total_fixed_fee_tax = $row1[1];
                        } elseif ($check_row == "tax_service_charges") {
                            $total_service_fee_tax = $row1[1];
                        } elseif ($check_row == "tax_delivery_charges") {
                            $deliver_fee_charges_tax = $row1[1];
                        } elseif ($check_row == "tax_markup_fee") {
                            $total_markup_fee_tax = $row1[1];
                        } elseif ($check_row == "container_charges_tax") {
                ...
                } elseif ($orderProduct->old_price > $orderProduct->price) {
        ...
        public function postVendorOrderStatusUpdate(Request $request)
        {
            ...
            try {
                ...
                } elseif ($order_status_option_id == 8) {
        ...
        public function sendOrderPushNotificationVendors($user_ids, $orderData, $header_code = '')
        {
            ...
            $client_preferences = ClientPreference::select('fcm_server_key', 'fcm_project_id', 'favicon', 'vendor_fcm_server_key')->first();
            if (!empty($devices) && !empty($client_preferences->fcm_project_id)) {
            ...
            if (!empty($vendorAppUserDevices)) {
                $data['tokens'] = $vendorAppUserDevices;
        ...
        public function sendStatusChangePushNotificationCustomer($user_ids, $orderData, $order_status_id, $header_code)
        {
            ...
            $client_preferences = ClientPreference::select('fcm_project_id', 'favicon')->first();
            if (!empty($devices) && !empty($client_preferences->fcm_project_id)) {
        ...
        public function sendOrderPushNotificationVendorReached($user_ids, $orderData, $header_code = '')
        {
            ...
            // Individual Vendor App User Token
            $vendorAppUserDevices = UserDevice::where('is_vendor_app', 1)->whereNotNull('device_token')->whereIn('user_id', $user_ids)->pluck('device_token')->toArray();
            if (!empty($vendorAppUserDevices) && !empty($client_preferences->fcm_project_id)) {
                $from = $client_preferences->fcm_project_id;
                $data['tokens'] = $vendorAppUserDevices;
                return sendFcmCurlRequest($data);
            }
        }
