    ...
    public function homepage(Request $request, $domain = '')
    {
        try {           
            ...
            if(\Config::get('filesystems.disks.s3.driver') == 's3') {
                $s3_url = 'https://'.\Config::get('filesystems.disks.s3.bucket').'.'.\Config::get('filesystems.disks.s3.driver').'.'.\Config::get('filesystems.disks.s3.region').'.amazonaws.com'.'/';
                if(\Config::get('filesystems.disks.s3.root') != null){
                    $s3_url = $s3_url . \Config::get('filesystems.disks.s3.root').'/';
                }
            }
            $image_const_arr = [
    ...
    public function searchProduct($langId, $keyword, $limit, $page, $action, $latitude, $longitude)
    {
        ...
        foreach ($keyword as $key=>$word) {
            $orderBy .= " WHEN pt.title LIKE '$word%' THEN $key " . 
                        " WHEN pt.title LIKE '%$word%' THEN $key + 10 ";
        }
        ...
        $products = Product::byProductCategoryServiceType($action)->with(['category.categoryDetail.translation' => function ($q) use ($langId) {
            ...
            ->where(function ($q) use ($keyword) {
                foreach ($keyword as $word) {
                    $q->orwhere('products.sku', ' LIKE', $word . '%')->orWhere('products.url_slug', 'LIKE', $word . '%')->orWhere('pt.title', 'LIKE', $word . '%')->orWhere('pt.title', 'LIKE', '%' . $word . '%');
                }
