...
class CartController extends BaseController
{
                ...
                foreach ($vendorData->vendorProducts as $pkey => $prod) {
                    ...
                    if(isset($prod->product) && !empty($prod->product)){
                      //  pr($prod->product);
                        ...
                        if ($prod->pvariant) {
                            $variantsData['price']              = $price_in_currency * $clientCurrency->doller_compare;
                            ...
                            if ( (in_array($action,['delivery','on_demand']) )  && ( $is_service_product_price_from_dispatch !=1 )) {
                                ...
                                if ($checkLastMile ) {
                                    ...
                                    if(isset($deliveries[0]))
                                    {
                                        ...
                                        // $vendorTotalDeliveryFee = $vendorTotalDeliveryFee * $clientCurrency->doller_compare;
                                        // $deliveryCharges_real = $deliveryCharges_real + $vendorTotalDeliveryFee;
                                        $deliveryCharges_real = $deliveryCharges_real + $vendorTotalPrimaryDeliveryFee;
                                    ...
                                    if(isset($vendorTotalDeliveryFee) && !empty($vendorTotalDeliveryFee)){
                                            ...
                                            CartDeliveryFee::updateOrCreate(['cart_id' => $cart->id, 'vendor_id' => $vendorData->vendor->id],['delivery_fee' => $vendorTotalDeliveryFee, 'primary_currency_delivery_fee' => $primaryDeliveryCharges, 'delivery_duration' => $deliveryDuration,'shipping_delivery_type' => $dtype[0]??'D','courier_id'=>$dtype[1]??'0']);
                                    }
    ...
    */
    ...
    public function getDeliveryOptions($vendorData, $preferences, $payable_amount, $address, $dispatcher_tags='', $totalRoute = '1')
    {
        ...
        if(Auth::user()){
            $currency_id = auth()->user()->currency;
        }
        ...
        if($preferences->static_delivey_fee != 1)
        {
            ...
            if (!empty($deliver_response_array[0])){
                ...
                $option[] = array(
                    ...
                    'rate' => $deliver_charge * $customerCurrency->doller_compare,
                    'primary_currency_rate' => $deliver_charge,
                    // 'rate' => $deliver_charge,
