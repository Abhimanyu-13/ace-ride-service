        ...
        if( (isset($preferences->is_hyperlocal)) && ($preferences->is_hyperlocal == 1) ){
            ...
            if(!empty($latitude) && !empty($longitude) ){
                ...
                if (isset($preferences->slots_with_service_area) && $preferences->slots_with_service_area == 1) {
                    // Get a base list of vendors that satisfy the initial conditions
                    $baseServiceAreaVendors = $serviceAreaVendors->get();
                    $filteredVendorIds = [];
    
                    foreach ($baseServiceAreaVendors as $vendor) {
                        if ($vendor->show_slot == 0) {
                            // Apply the specific condition for vendors with show_slot == 0
                            $filteredVendorQuery = Vendor::where('id', $vendor->id)
                            ...
                            $filteredVendor = $filteredVendorQuery->first();
                            if ($filteredVendor) {
                                $filteredVendorIds[] = $filteredVendor->id;
                            }
                        } else {
                            // Directly add vendors without the show_slot condition
                            $filteredVendorIds[] = $vendor->id;
                        }
                    }
    
                    // Reset the service area vendors query to only include the filtered IDs
                    $serviceAreaVendors = Vendor::whereIn('id', $filteredVendorIds);
                }
                // if (isset($preferences->slots_with_service_area) && ($preferences->slots_with_service_area == 1)) {
