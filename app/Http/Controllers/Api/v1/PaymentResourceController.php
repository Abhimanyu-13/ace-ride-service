...
class PaymentResourceController extends BaseController
{
    ...
    public function createPaymentIntent(Request $request)
    {
        ...
        if (!$saved_payment_method) {
            $customerResponse = \Stripe\Customer::create(array(  
                ...
                'address' => [
                    ...
                    'state' => "state",  // State (if available)
                    'country' => "IN",  // Country (ensure it's a valid country code, like 'IN' for India)
                ],
            ));  
        ...
        // Ensure the PaymentMethod is attached to the Customer
        ...
        if($payment_form == 'cart'){
            ...
            if($address_id){
                ...
                $user_address = UserAddress::where('user_id', $user->id)->where('is_primary', 1)->first();
            }
            ...
            $postdata['shipping']['address']['country'] = ($user_address) ? ($user_address->country_code != "") ? $user_address->country_code : 'N/A' : 'N/A';
        ...
        \Log::info('Stripe Payment Intent Request: '.json_encode($postdata));
