        ...
        try {
            ...
            if (Auth::user()->is_superadmin == 0 || $request->manager_id) {
                ...
                $vendor_orders = OrderVendor::with(['user','vendor'])->whereHas('orderDetail',function ($query){
                    ...
                    $query->orWhere(function ($q2) {
                        $q2->whereIn('payment_option_id', [1,38]);
                    })->whereHas('vendors.vendor.permissionToUser', function ($query) {
                        $query->where('user_id', Auth::user()->id);
                    });
                });
            ...
            $previousweek_startdate = Carbon::now()->startOfWeek()->subWeek()->startOfDay();
            $previousweek_enddate = Carbon::now()->startOfWeek()->subWeek()->endOfWeek()->endOfDay();
            
            $dataSum = $data->whereBetween('created_at', [$previousweek_startdate, $previousweek_enddate])
                ->selectRaw('DATE(created_at) as date, SUM(payable_amount) as daily_total')
                ->groupBy('date')
                ->orderBy('date')
                ->get()
                ->pluck('daily_total', 'date'); 

            // Initialize a result array with all days of the week set to 0.0
            $defaultRevenue = collect(range(0, 6))->mapWithKeys(function ($day) use ($previousweek_startdate) {
                $date = $previousweek_startdate->copy()->addDays($day)->toDateString();
                return [$date => 0.0];
            });

            // Merge the database results and ensure all values are floats
            $previousweek_revenue_daywise = $defaultRevenue
                ->merge($dataSum)
                ->map(fn($value) => (float) $value) // Ensure all values are cast to float
            ...
            // }
            // dd($previousweek_revenue_daywise);
            
            # Current week day wise revenue total

            $currentweek_startdate = Carbon::now()->startOfWeek()->startOfDay();
            $currentweek_enddate = Carbon::now()->endOfWeek()->endOfDay();

            // Fetch data for current week, grouped by date
            $currentWeekDataSum = $data1->whereBetween('created_at', [$currentweek_startdate, $currentweek_enddate])
                ->selectRaw('DATE(created_at) as date, SUM(payable_amount) as daily_total')
                ->groupBy('date')
                ->orderBy('date')
                ->get()
                ->pluck('daily_total', 'date'); // Key = date, Value = total

            // Initialize a result array with all days of the current week set to 0.0
            $currentDefaultRevenue = collect(range(0, 6))->mapWithKeys(function ($day) use ($currentweek_startdate) {
                $date = $currentweek_startdate->copy()->addDays($day)->toDateString();
                return [$date => 0.0];
            });

            // Merge the database results and ensure all values are floats
            $currentweek_revenue_daywise = $currentDefaultRevenue
                ->merge($currentWeekDataSum)
                ->map(fn($value) => (float) $value) // Ensure all values are cast to float
                ->values() // Reset keys to numeric
                ->toArray();
