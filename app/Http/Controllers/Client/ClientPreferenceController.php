...
class ClientPreferenceController extends BaseController
{
    ...
    public function index()
    {
        ...
        $ClientPreference = ClientPreference::where('client_code', (string)"$client->code")->first();
    ...
    public function getCustomizePage(ClientPreference $clientPreference)
    {
        ...
        $ClientPreference = ClientPreference::where('client_code', (string)$client->code)
            ->first();
    ...
    public function referandearnUpdate(Request $request, $code)
    {
        ...
        $preference = ClientPreference::where('client_code', (string)Auth::user()->code)->first();
        if ($preference) {
    ...
    public function update(Request $request, $code)
    {
        ...
        $preferenceset = ClientPreference::where('client_code', (string)Auth::user()->code)->first();
        ...
        if (isset($request->threshold_submit_btn) && !empty($request->threshold_submit_btn)) {
            if (isset($request->threshold_submit_btn) && !empty($request->threshold_submit_btn)) {

                $preferenceset->need_threshold_service = ($request->has('need_threshold_service') && $request->need_threshold_service == 'on') ? 1 : 0;
                $preferenceset->need_threshold_amount = $request->need_threshold_amount;
                $preferenceset->laundry_discount_amount = $request->laundry_discount_amount;
            } else {
                $preferenceset->need_threshold_service =  ($request->has('need_threshold_service') && $request->need_threshold_service == 'on') ? 1 : 0;
            }
        }
    ...
    public function toggleDatabase(Request $request)
    {
        ...
        Config::set("database.connections.btb_royo_orders", $default);
        FacadesDB::setDefaultConnection('btb_royo_orders');
