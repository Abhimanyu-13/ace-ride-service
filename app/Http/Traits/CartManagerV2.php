          ...
          // $sub_total+=$opt_price_in_currency;
            ...
            foreach ($cartData as $ven_key => $vendorData) {
               ...
               // if(!empty($user)){
                ...
                $vendorTotalPrimaryDeliveryFee = 0;
                        ...
                        if ( (in_array($action,['delivery','appointment','on_demand']) ) && ( $is_service_product_price_from_dispatch !=1 ) ) {
                           ...
                           // if ((!empty($prod->product->Requires_last_mile) && ($prod->product->Requires_last_mile == 1))  ) {
                                ...
                                if (isset($deliveries[0])) {
                                    ...
                                    if($code) {
                                        ...
                                        foreach ($new as $rate) {
                                            ...
                                            $primaryDeliveryCharges = $rate['primary_currency_rate'];
                                        }
                                        if ($deliveryCharges) {
                                            $deliveryCharges = $rate['rate'];
                                            $primaryDeliveryCharges = $rate['primary_currency_rate'];
                                            ...
                                            $primaryDeliveryCharges = $deliveries[0]['primary_currency_rate'];
                            ...
                            if($prod->product->individual_delivery_fee == 1) {
                                ...
                                $quantity_primaryDeliveryCharges = $primaryDeliveryCharges*$prod->quantity;
                                $vendorTotalDeliveryFee = $vendorTotalDeliveryFee + $quantity_deliveryCharges;
                                $vendorTotalPrimaryDeliveryFee = $vendorTotalPrimaryDeliveryFee + $quantity_primaryDeliveryCharges;
                                CartProduct::where('id', $prod->id)->update(['product_delivery_fee'=>$quantity_primaryDeliveryCharges]);
                                $prod->product_delivery_fee = $quantity_primaryDeliveryCharges;
                            }else{
                                if($if_previousdeliveryfee_added == 0 && $deliveryCharges > 0){
                                    $vendorTotalDeliveryFee = $vendorTotalDeliveryFee + $deliveryCharges;
                                    $vendorTotalPrimaryDeliveryFee = $vendorTotalPrimaryDeliveryFee + $primaryDeliveryCharges;
                                    $if_previousdeliveryfee_added = 1;
                                }
                            }

                            // if ($vendorTotalDeliveryFee > 0) {
                            //     if($customerCurrency){
                            //         $vendorTotalDeliveryFee = $vendorTotalDeliveryFee * $customerCurrency->doller_compare;
                            //     }
                            // }
                            $deliveryCharges_real += $vendorTotalDeliveryFee;
                            $primaryDeliveryCharges_real += $vendorTotalPrimaryDeliveryFee;
                            if (isset($deliveryCharges_real) && !empty($deliveryCharges_real)) {
                                $dtype = explode('_', $code);
                                CartDeliveryFee::updateOrCreate(['cart_id' => $cart->id, 'vendor_id' => $vendorData->vendor->id], ['delivery_fee' => $deliveryCharges_real,'primary_currency_delivery_fee' => $primaryDeliveryCharges_real,'shipping_delivery_type' => $dtype[0]??'D','courier_id'=>$dtype[1]??'0']);
                            }
                ...
                if ($vendorData->vendor->service_charge_amount > 0) {
                    if($customerCurrency){
                        $vendorData->vendor->service_charge_amount = $vendorData->vendor->service_charge_amount * $customerCurrency->doller_compare;
                    }
                }
