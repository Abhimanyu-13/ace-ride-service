<div>
    <a class="card scale-effect text-center" href="{{ $product->vendor_slug }}/product/{{ $product->url_slug }}">
        <label class="product-tag">@if($product->title != 0) {{$product->title}} @else {{$homePageLabel->translations ? $homePageLabel->translations[0]['title'] : $homePageLabel->title}}@endif </label>
