    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <div class="product_form">
            @csrf
            <div class="row mb-2">
                <input type="hidden" value="" name="user_id" id="form_userid"/>
                <div class="col-12 mb-2">
                    {!! Form::label('title', __('Phone Number'),['class' => 'control-label']) !!}
                    {!! Form::text('title', ' ', ['class'=>'form-control', 'id' => 'userphonenumber', 'placeholder' => '+919041619007', 'required' => 'required']) !!}
                </div>
                <div class="col-12 mb-12">
                    <p class="userfounornot"></p>
                </div>
                <div class="col-md-6 col-12 mb-2">
                    {!! Form::label('title', __('Name'),['class' => 'control-label']) !!}
                    {!! Form::text('title', 'Text', ['class'=>'form-control', 'id' => 'formusername', 'placeholder' => 'Apple iMac', 'required' => 'required']) !!}
                </div>
                <div class="col-md-6 col-12 mb-2">
                    {!! Form::label('title', __('Email'),['class' => 'control-label']) !!}
                    {!! Form::text('title', 'Text', ['class'=>'form-control', 'id' => 'formuseremail', 'placeholder' => 'Apple iMac', 'required' => 'required']) !!}
                </div>
                <div class="col-md-12 col-12 mb-2">
                    <input type="button" class="btn btn-primary" value="Regsiter User" id="register_userclick"/>
                </div>
                <div class="col-md-12 success_message"></div>
                <div class="col-md-6 col-12 mb-2">
                    {!! Form::label('title', __('Category'),['class' => 'control-label']) !!}
                    <select name="product_category" id="category_selected" disabled class="form-control">
                ...
                <div class="col-md-6 col-12 mb-2">
                    {!! Form::label('title', __('Service'),['class' => 'control-label']) !!}
                    <select name="product_service" id="service_selected" disabled class="form-control">
                ...
                <div class="col-md-12 col-12 mb-2">
                    {!! Form::label('title', __('Addon'),['class' => 'control-label']) !!}
                    <div class="addon_wrapper">

                    </div>
                </div>
                <div class="col-md-6 col-12 mb-2">
                    {!! Form::label('title', __('Total'),['class' => 'control-label']) !!}        
                </div>
                <div class="col-md-6 col-12 mb-2 service_price">
                    122      
                </div>
                <input type="hidden" value="" class="servicepriceselected" />
                <input type="hidden" value="" class="serviceidselected" />
                <input type="hidden" value="" class="product_variant_id" />
                <input type="hidden" value="" class="product_vendor_id" />
                <div class="col-md-12">
                    <input type="button" value="submit" class="btn btn-primary" id="servicecresatedasdasd"/>
                </div>
            </div>
        </div>
      </div>
    ...
    <div class="row order-list-spinner">
        <div class="tab-product pl-2 pr-2 flex-grow-1">
            <div class="item">
                ...
                <div class="col-md-3" style="    position: absolute; right: 0; top: 0; text-align: right;">
                    <a href="#" data-toggle="modal" data-target="#exampleModal">Add Product</a>
                </div>
                @endif
...
<script>
    ...
    $(document).ready( function() {
        $('#userphonenumber').on('change focusin focusout' , function() {
            var as = $(this).val();
            if(!as){
                return false
            }
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if(response != 'User Not found'){
                        $('#form_userid').val(response.id);      
                        $('#formusername').val(response.name);    
                        $('#formuseremail').val(response.email);
                        $('.userfounornot').html('User Found');
                        $('#register_userclick').hide();
                        $('#category_selected').prop('disabled',false)
                    } else {
                        $('.userfounornot').html(response);
                        $('#form_userid').val(' ');      
                        $('#formusername').val(' ');    
                        $('#formuseremail').val(' ');
                        $('#category_selected').prop('disabled',true)
                        $('#register_userclick').show();
                    }
...
<script>
   $(document).ready( function() {
       $('#category_selected').change( function() {
            var catid = $(this).val();
            const url = '/api/v1/v2/category/'+catid;
            $.ajax({
                ...
                success: function(response) {
                    ...
                    var select = $('#service_selected');

                    select.empty();

                    select.append($('<option selected disabled>Select a Service</option>'));
                    // Iterate through the data and append an option for each item
                    $.each(response.data.listData.data, function(index, value) {
                        console.log(value);
                        select.append($('<option>', {
                            value: value.id,
                            text: value.translation_title
                        }));
       ...
       $('#service_selected').change( function() {
            var catid = $(this).val();
            const url = '/api/v1/product/'+catid;
            $.ajax({
                ...
                success:  async function(response) {
                    ...
                    $('.product_vendor_id').val(response.data.products.vendor_id);

                    $.each(response.data.products.variant , async function(index, value) {

                        
                        if(index < 1){
                            $('.service_price').html(value.actual_price);

                            $('.servicepriceselected').val(value.price)

                            $('.serviceidselected').val(value.product_id)

                            $('.product_variant_id').val(value.id)
                        }
                        
                    });


                    var container = $('.addon_wrapper');
                    
                    
                    container.empty();

                    await $.each(response.data.products.add_on , async function(index, value) {
                        var html1 = value.title;
  
                        $finalhtml = '<h3>'+ html1  +'</h3>';

                        container.append($finalhtml);

                        await $.each(value.setoptions , function(index, value) {

                         
                            var newtile = $.trim(value.title).toLowerCase().replace(/\s/g, '');
                            var checkbox = $('<input>', {
                                type: 'checkbox',
                                value: value.id,
                                class: 'addonchekbox',
                                name: 'addonchekbox' // You can use the same name for checkboxes you want to be in the same group
                            });
                            ...
                            var input = $('<input>', {
                                ...
                                value: value.price,
                                id: 'price'+value.id,
                                name: 'price' // You can use the same name for checkboxes you want to be in the same group
                            });

                            var input1 = $('<input>', {
                                type: 'hidden',
                                value: value.addon_id,
                                id: 'addon_id'+value.id,
                                name: 'addonoption_id' // You can use the same name for checkboxes you want to be in the same group
                            });

                            // Create a label for the checkbox
                            var label = $('<label>', {
                                text: value.title,
                                for: value.id // Connect label to the checkbox using the 'for' attribute
                            });
                            ...
                            container.append('<br>'); // Add line break for better spacing


                        })
                        
                    });
                    //Add price in total price
                         $('.addonchekbox').change( function() {
                        console.log("Here")
                        
                        if ($(this).prop('checked')==true){ 
                            var asd = '#price'+$(this).val();
                            var as = $(asd).val();
                            var ds = $('.service_price').html();

                            var asa = parseFloat(ds) + parseFloat(as);

                            $('.service_price').html(asa);
       ...
       $('#register_userclick').click( function() {

            
           var username =  $('#formusername').val();
           if(!username){
                $('.userfounornot').val('user name is required')
                return false;
           }
           var emailcheck =  $('#formuseremail').val();
           if(!emailcheck){
                $('.userfounornot').val('email is required')
                return false;
           }
           ...
           $.ajax({
            ...
            url: "{{ route('user.registeruserbyphone') }}",
            type: "POST",
            data: {                          
                username: username,
            ...
            success: function(response) {
                ...
                if(response){
                    $('#register_userclick').hide();
                    $('.success_message').html('User register successfully')
                    $('#category_selected').prop('disabled',false)
                }
       ...
       $('#servicecresatedasdasd').click( function() {
            var service_id = $('.serviceidselected').val();
            var product_variant_id = $('.product_variant_id').val();
            var product_vendor_id = $('.product_vendor_id').val();
            var addonID = [] ;
            i = 0;
            $('.addonchekbox').each( function() {
                if ($(this).prop('checked')==true){ 
                    var as = '#addon_id'+$(this).val();
                    addonID[i++] = $(as).val()
                }
            })
            //console.log(addonID);
            const url = '/add/product/cart';
            $.ajax({
