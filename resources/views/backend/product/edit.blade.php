...
<!-- <div class="container-fluid">
    <div class="row">
        <div class="col-lg-6">
            <div class="card-box product-list">
                <div class="d-flex align-items-center justify-content-between bg-light p-2 mb-4">
                    <h5 class="text-uppercase mt-0 mb-0 ">Product Addon</h5>
                    <button class="btn btn-info waves-effect waves-light" data-toggle="modal" data-target="#exampleModal">Addon</button>
                </div>
                <ul class="pl-0">
                    <table class="table table-centered dataTable table-nowrap table-striped w-100 no-footer dtr-inline collapsed" id="vendor_product_table" role="grid" aria-describedby="vendor_product_table_info">
                        <thead>
                            <tr role="row">
                                <th class="sorting_disabled" rowspan="1" colspan="1" aria-label="#">#</th>
                                <th class="sorting" tabindex="0" aria-controls="vendor_product_table" rowspan="1" colspan="1" aria-label="Name: activate to sort column ascending">Name</th>
                                <th class="sorting_disabled" rowspan="1" colspan="1" aria-label="Category">Price</th>
                                <th class="sorting_disabled" rowspan="1" colspan="1" aria-label="Brand">Code</th>                            
                                <th class="sorting_disabled" rowspan="1" colspan="1" aria-label="Action">Action</th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach($productadoncustom as $productadoncustoms)
                            @if($productadoncustoms)
                            <tr role="row" class="odd">
                                <td >
                                    <input type="checkbox" class="single_product_check" name="product_id[]" id="single_product" value="2">
                                </td>
                                <td>
                                    {{ $productadoncustoms->name }}
                                </td>
                                <td>
                                    {{ $productadoncustoms->price }}
                                </td>
                                <td>
                                    {{ $productadoncustoms->item_code }}
                                </td>
                                <td>
                                    <div class="form-ul" style="width: 60px;">
                                    <div class="inner-div">
                                        <form id="deleteproduct_2" method="POST" action="{{route('deleteaddons.product.clients', $productadoncustoms->id)}}">
                                            @csrf
                                            <input type="hidden" name="_method" value="DELETE">
                                            <div class="form-group">
                                                <button type="button" class="btn btn-primary-outline action-icon delete-product" data-destroy_url="{{route('deleteaddons.product.clients', $productadoncustoms->id)}}" data-rel="2"><i class="mdi mdi-delete"></i></button>
...
<!-- <div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Addon Products</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      ...
      @if($client_preference_detail->business_type == 'laundry' )
        <div class="col-12">
            <form action="{{route('saveaddons.product.clients')}}" enctype="multipart/form-data" method="post" class="product_form">
                @csrf
                <div class="row mb-2 mx-0">
                    <input type="hidden" value="{{ $product->id }}" name="product_id" />
                    <div class="col-12 mb-2">
                        {!! Form::label('title', __('Name'),['class' => 'control-label']) !!}
                        {!! Form::text('addonname', '', ['class'=>'form-control', 'id' => 'addonname', 'placeholder' => 'Apple iMac', 'required' => 'required']) !!}
                    </div>
                    <div class="col-12 mb-2">
                        {!! Form::label('title', __('Price'),['class' => 'control-label']) !!}
                        {!! Form::number('price', '', ['class'=>'form-control', 'id' => 'price', 'placeholder' => '0.00', 'required' => 'required']) !!}
                    </div>
                    <div class="col-12 mb-2">
                        {!! Form::label('title', __('Code'),['class' => 'control-label']) !!}
                        {!! Form::text('item_code', '', ['class'=>'form-control', 'id' => 'item_code', 'placeholder' => 'ITC', 'required' => 'required']) !!}
                    </div>
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.status == 'Success') {
                        ...
                        }, response.message, "top-right", "#ab0535", "error");
                    }
                },
                ...
                if (response.status = 'Success') {
                    if (response.data.file_type == "selector") {
                        ...
                        $(options).each(function(index, value) {
                            ...
                            section_id = section_id + 1;
                            $('#vendor-selector-datatable #table_body').append(modified_temp({
                                id: section_id,
                                data: value
                            }));
