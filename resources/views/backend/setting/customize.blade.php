...
{{-- vendoe typs section aline by harbans singh :) --}}
    <div class="row col-spacing">
        ...
        @if($client_preference_detail->business_type == 'food_grocery_ecommerce' || $client_preference_detail->business_type == 'taxi' || $client_preference_detail->business_type == '' || $client_preference_detail->business_type == 'super_app' && $client_preference_detail->pick_drop_check == 1 )
        ...
        <div class="col-xl-3 col-lg-3 mb-3">
            ...
            <form method="POST" action="{{route('configure.update', Auth::user()->code)}}">
                ...
                <div class="card-box h-100">
                    <div class="d-flex align-items-center justify-content-between mb-2">
                        <h4 class="header-title mb-0">{{ __("Free Service") }}</h4>
                        <button class="btn btn-info d-block" type="submit" name="threshold_submit_btn" value ="1"> {{ __("Save") }} </button>
                    </div>
                    <p class="sub-header">{{ __("Add the threshold service for free service ") }}</p>
                    <div class="row">
                        <div class="col-12">
                            <div class="form-group mb-0">
                                <div class="form-group mb-0 switchery-demo">
                                    <label for="need_threshold_service" class="mr-3">{{ __("Enable") }}</label>
                                    <input data-plugin="switchery" name="need_threshold_service" id="need_threshold_service" class="form-control" data-color="#43bee1" type="checkbox" @if((isset($preference) && $preference->need_threshold_service == '1')) checked @endif >
                                </div>
                            </div>
                            <div class="form-group mt-3 mb-0 laundryServiceFields" style="{{((isset($preference) && $preference->need_threshold_service == '1')) ? '' : 'display:none;'}}">
                                <label for="need_threshold_amount">{{ __("Threshold Amount") }}</label>
                                <input type="text" name="need_threshold_amount" id="need_threshold_amount" placeholder="0" class="form-control" value="{{ old('need_threshold_amount', $preference->need_threshold_amount ?? '')}}">
                            </div>
                            <div class="form-group mt-3 mb-0 laundryServiceFields" style="{{((isset($preference) && $preference->need_threshold_service == '1')) ? '' : 'display:none;'}}">
                                <label for="laundry_discount_amount">{{ __("Discount Amount") }}</label>
                                <input type="text" name="laundry_discount_amount" id="laundry_discount_amount" placeholder="" class="form-control" value="{{ old('laundry_discount_amount', $preference->laundry_discount_amount ?? '')}}">
                                @if($errors->has('laundry_discount_amount'))
                                <span class="text-danger" role="alert">
                                    <strong>{{ $errors->first('laundry_discount_amount') }}</strong>
                                </span>
                                @endif
                            </div>
                        </div>
